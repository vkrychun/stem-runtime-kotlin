...
Consumers should track the latest release via the Maven dependency declared in `build.gradle.kts`.
...
- The Android API level, Android Gradle Plugin version, and Kotlin / Compose versions used to reproduce.
...
- `secured` repository / `EncryptedSharedPreferences` handling flaws.
- Native (`.so`) integrity-check bypass that allows an unlicensed build to suppress the watermark.
